...
function BookReader() {
    ...
    this.imagesBaseURL = '/public/images/default/images/';
...
(function($) {
    ...
    BookReader.prototype.onePageCalculateReductionFactors = function(width, height) {
        this.onePage.reductionFactors = this.reductionFactors.concat(
            [{
                reduce: this.onePageGetAutofitWidth(),
                autofit: 'width'
            }, {
                reduce: this.onePageGetAutofitHeight(),
                autofit: 'height'
            }]);
    ...
    BookReader.prototype.flipLeftToRight = function(newIndexL, newIndexR) {
        ...
        $(this.leafEdgeTmp).animate({
            left: gutter
        }, this.flipSpeed, 'easeInSine');
        ...
        $(this.prefetchedImgs[leftLeaf]).animate({
            width: '0px'
        }, self.flipSpeed, 'easeInSine', function() {

            //console.log('     and now leafEdgeTmp to left: gutter+newWidthR ' + (gutter + newWidthR));
            $(self.leafEdgeTmp).animate({
                left: gutter + newWidthR + 'px'
            }, self.flipSpeed, 'easeOutSine');

            $('#BRgutter').css({
                left: (gutter - self.twoPage.bookSpineDivWidth * 0.5) + 'px'
            });

            //console.log('  animating newIndexR ' + newIndexR + ' to ' + newWidthR + ' from ' + $(self.prefetchedImgs[newIndexR]).width());
            $(self.prefetchedImgs[newIndexR]).animate({
                width: newWidthR + 'px'
            }, self.flipSpeed, 'easeOutSine', function() {
    ...
    BookReader.prototype.search = function(term) {
        ...
        $.ajax({
            url: url,
            dataType: 'jsonp',
            jsonpCallback: 'br.BRSearchCallback'
        });
    }
    ...
    BookReader.prototype.initToolbar = function(mode, ui) {
        ...
        $('#BRinfo .BRfloatTitle a').attr({
            'href': this.bookUrl
        }).text(this.bookTitle).addClass('title');
        ...
        $('.BRnavCntl').click(
            function() {
                if ($('#BRnavCntlBtm').hasClass('BRdn')) {
                    ...
                    $('#BRnavCntlBtm.BRnavCntl').animate({
                        height: '45px'
                    });
                    $('.BRnavCntl').delay(1000).animate({
                        opacity: .25
                    }, 1000);
                } else {
                    $('#BRtoolbar').animate({
    ...
    BookReader.prototype.gotOpenLibraryRecord = function(self, olObject) {
        ...
        if (olObject) {
            ...
            if (self.theme == 'ol') {
                ...
                $('#BRreturn a').attr({
                    'href': self.bookUrl,
                    'title': "Go to this book's page on Open Library"
                });
    ...
    BookReader.prototype.ttsStartCB = function(data) {
        ...
        var snd = soundManager.createSound({
            ...
            onbufferchange: function() {
                    if (false == this.isBuffering) this.br.removeProgressPopup();
                } //fires in FF and IE9
        });
        snd.br = this;
        snd.load();
    ...
    BookReader.prototype.ttsPlay = function() {
        ...
        //play current chunk
        if (false == this.ttsBuffering) {
            soundManager.play('chunk' + this.ttsIndex + '-' + this.ttsPosition, {
                onfinish: function() {
                    br.ttsNextChunk();
                }
            });
